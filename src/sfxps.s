...
; wpset F800,1,w,wpdata==39, { printf "Playing SFX ($%04X, $%02X) on channel %d", bc, (iy & FF), e; go }
; wpset F800,1,w,wpdata==3A && (e != 0), { printf "Sample(s) finished: %02X", e; go}
SFXPS_update:
    push af
    push de
...
; wpset F800,1,w,wpdata==39, { printf "Playing SFX ($%04X, $%02X) on channel %d", bc, (iy & FF), e }
SFXPS_play_sfx:
    push af
    push bc
    push hl
...
SFXPS_retrigger_sfx:
    ...
    push de
    push ix
        ; The channel is *always* found even on real hardware, 
        ; it just isn't actually played by the YM2610 when
        ; playing SFX really close to each other.
